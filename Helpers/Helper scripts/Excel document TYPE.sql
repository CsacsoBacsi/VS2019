...
CREATE INDEX IDX_EXCELDOCUMENTSTORE ON EXCELDOCUMENTSTORE
(OBJECT_ID, SEGMENT);
/


CREATE OR REPLACE
TYPE BODY C10879.EXCELDOCUMENTTYPE AS 
...
BEGIN
   ...
   USING SELF.object_id,p_index,p_segment,p_value,lengthb(v_value)+2;
...
IS
  ...
  v_row VARCHAR2(100) := '<Row ss:StyleID="<style>">';
...
MEMBER PROCEDURE createStyle(p_style_id     VARCHAR2 := NULL,
                         ...
                         p_borders          VARCHAR2 := NULL,
                         p_weight           VARCHAR2 := NULL)
IS
...
BEGIN
        ...
        IF p_style_id IS NOT NULL THEN
          ...
          -- Borders
          IF p_borders IS NOT NULL THEN
             borders := '<Borders>' || NL_CHAR ;
             IF p_borders = '1' THEN
                 borders := borders || '<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             END IF ;

             IF p_borders = '2' THEN
                 borders := borders || '<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             END IF ;
             ...
             IF p_borders = '4' THEN
                 borders := borders || '<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             END IF ;
             ...
             IF p_borders = '8' THEN
                 borders := borders || '<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             END IF ;
             ...
             IF p_borders = '10' THEN
                 borders := borders || '<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             END IF ;

             IF p_borders = '11' THEN
                 borders := borders || '<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             ...
             IF p_borders = '12' THEN
                 borders := borders || '<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             END IF ;

             IF p_borders = '13' THEN
                 borders := borders || '<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="' || p_weight || '"/>' || NL_CHAR ;
                 borders := borders || '</Borders>' || NL_CHAR ;
             ...
             v_style_tag := REPLACE (v_style_tag,'<Borders>', borders) ;
          ELSE
             v_style_tag := REPLACE (v_style_tag,'<Borders>',NULL) ;
          END IF;
...
IS
   ...
   v_width  NUMBER(12)     := 0;
   v_coltag VARCHAR2(4000) := '<Column ss:Index="<index>" ss:AutoFitWidth="0" ss:Width="<width>"/>';  
...
MEMBER PROCEDURE addCell(p_col_index  VARCHAR2 := NULL,
                         ...
                         p_merge      VARCHAR2 := NULL) 
IS

      v_cell VARCHAR2(32000) := '<Cell ss:MergeAcross="<merge>" ss:Index="<index>" ss:Formula="=<formula>" ss:StyleID="<style>"><Data ss:Type="<datatype>"><p_data></Data></Cell>';
...
BEGIN
       ...
       IF p_merge IS NOT NULL THEN
          ...
          v_cell := REPLACE (v_cell,'<merge>',p_merge);

       ELSE

          v_cell := REPLACE(v_cell,' ss:MergeAcross="<merge>"',NULL);
