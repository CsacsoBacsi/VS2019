...
*/
	...
	if (deli->isLeafNode || dtype[dtypeInd - 1].deltype[1] == 'c') { // Is it a leaf node? Or the previous deletion involved merging
		if (tree[deli->nodeIndex].count > MIN_KEYS) {			// Will node have enough keys remaining after the deletion
			...
			else if (tree[deli->leftSibling].count > MIN_KEYS) {// Any siblings have more than min amount of keys?
				deli->delType [0] = '2' ;						// Type 2a: leaf node, with not enough values, left sibling can help out
				deli->delType [1] = 'a' ;
			}
			else if (tree[deli->rightSibling].count > MIN_KEYS) {
...
*/
	...
	thisNode->key[deli->position] = pred.key ;
...
*/
	...
	thisNode->key[deli->position] = suc.key ;
...
*/
	...
	thisNode->key[deli->position] = pred.key ;					// Replace parent key with this nominated key
	thisNode->rowNum[deli->position] = tree[pred.nodeIndex].rowNum[pred.position] ;
...
*/
	...
	thisNode->key[deli->position] = suc.key ;					// Replace parent key with this nominated key
	thisNode->rowNum[deli->position] = tree[suc.nodeIndex].rowNum[suc.position] ;
...
Predecessor _GetPred (int nodeIndex, int pos) {					// Recursive pair of GetPred
	...
	if (thisNode->nodePointer[pos] == -1) {						// Reached a leaf
		...
		if (leftSibling != -1) {
			pred.leftSiblingCount = tree[leftSibling].count ;
		}
		else {
		...
		if (rightSibling != -1) {
			pred.rightSiblingCount = tree[rightSibling].count ;
		}
		else {
		...
		return _GetPred (thisNode->nodePointer[pos], tree[thisNode->nodePointer[pos]].count) ;
	}
}
